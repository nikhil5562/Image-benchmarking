...
def plot_one_box(x, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=2, orig_shape=None):
    ...
    cv2.rectangle(im, c1, c2, color, thickness=tl*2, lineType=cv2.LINE_AA)  # Increase line thickness
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]  # Increase font scale
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(im, c1, c2, color, -1, cv2.LINE_AA)  # filled

        # Get the average color of the bounding box area
        box_area = im[c1[1]:c2[1], c1[0]:c2[0]]
        if box_area.size > 0:  # Check if the bounding box area is not empty
            avg_color = np.mean(box_area, axis=(0, 1))
            label_color = (0, 0, 0) if sum(avg_color) > 128 else (255, 255, 255)
        else:
            label_color = (255, 255, 255)  # Default to white color if the bounding box area is empty

        cv2.putText(im, label, (c1[0], c1[1] - 2), 0, tl / 3, label_color, thickness=tf//2, lineType=cv2.LINE_AA)
    if kpt_label:
